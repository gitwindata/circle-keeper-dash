...
CREATE POLICY "Hairstylists can view their pending members" ON pending_members
FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Hairstylists can create pending members" ON pending_members
...
CREATE POLICY "Admins can view all pending members" ON pending_members
FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE user_profiles.id = auth.uid() 
...
CREATE POLICY "Admins can update pending members" ON pending_members
FOR UPDATE USING (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE user_profiles.id = auth.uid() 
