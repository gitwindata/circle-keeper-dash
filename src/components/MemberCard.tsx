...
export function MemberCard({
  ...
  const MembershipIcon =
    membershipIcons[member.membership_tier as keyof typeof membershipIcons];
  const displayName =
    member.user_profile?.full_name || `Member ${member.id.slice(0, 8)}`;
              ...
              <div className="flex items-center space-x-2 mt-1">
                <Badge
                  ...
                  className={cn(
                    "text-xs",
                    membershipColors[
                      member.membership_tier as keyof typeof membershipColors
                    ]
                  )}
                >
      ...
      <CardContent className="space-y-4">
        ...
        {member.last_visit_date && (
          <div className="flex items-center space-x-2 text-sm text-gray-600 bg-gray-50 rounded-lg p-2">
            ...
            <span>
              Last visit:{" "}
              {new Date(member.last_visit_date).toLocaleDateString("id-ID")}
            </span>
          </div>
        )}
