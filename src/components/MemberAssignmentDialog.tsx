...
const MemberAssignmentDialog = ({
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const filtered = unassignedMembers.filter(member =>
        member.user_profile.full_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        member.user_profile.email.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleAssignMember = async (memberId: string, memberName: string) => {
    try {
      setAssigning(true);
      await memberHelpers.assignExistingMemberToHairstylist(memberId, hairstylistId);
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to assign member. Please try again.",
        variant: "destructive"
      });
    } finally {
      setAssigning(false);
            ...
            {loading ? (
              <div className="flex items-center justify-center py-8">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Loading available members...</p>
                </div>
              </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">
                  {searchQuery ? 'No members match your search' : 'No unassigned members available'}
                </p>
                {!searchQuery && (
                ...
                <Card key={member.id} className="hover:shadow-md transition-shadow">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-medium">{member.user_profile.full_name}</h3>
                            <Badge className={getMembershipBadgeColor(member.membership_tier)}>
                      ...
                      <Button
                        onClick={() => handleAssignMember(member.id, member.user_profile.full_name)}
                        disabled={assigning}
                        size="sm"
                      >
